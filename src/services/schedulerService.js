...
class SchedulerService {
  ...
  // Schedule test alert for 5:55 PM TODAY (for testing purposes)
  scheduleTestAlert() {
    cron.schedule('55 17 * * *', async () => {
      console.log('🧪 TEST ALERT: Starting outbreak broadcast at 5:55 PM IST');
      ...
      try {
        ...
        if (nationalAlert) {
          ...
          // Broadcast to all users
          await broadcastService.broadcastNationalAlert(nationalAlert);
          console.log('✅ TEST ALERT: Demo alert broadcast completed at 5:55 PM');
        } else {
          console.log('ℹ️ TEST ALERT: No national outbreak alert to broadcast at 5:55 PM');
        }
        
      } catch (error) {
        console.error('❌ Error in 5:55 PM test outbreak broadcast:', error);
        
        // Send error notification to admin (if configured)
        await this.notifyAdminOfError('5:55 PM Test Outbreak Broadcast Failed', error);
      }
    }, {
      timezone: "Asia/Kolkata"
    });

    console.log('🧪 TEST ALERT: Outbreak broadcast scheduled for 5:55 PM IST TODAY');
  }
  ...
  getStatus() {
    ...
    return {
      ...
      jobs: {
        ...
        testAlert: {
          schedule: '55 17 * * *',
          description: 'TEST ALERT: Outbreak broadcast at 5:55 PM IST TODAY',
