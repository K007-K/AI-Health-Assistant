...
class SchedulerService {
  ...
  // Schedule test alert for 5:45 PM TODAY (for testing purposes)
  scheduleTestAlert() {
    cron.schedule('45 17 * * *', async () => {
      console.log('🧪 TEST ALERT: Starting outbreak broadcast at 5:45 PM IST');
      ...
      try {
        ...
        if (nationalAlert) {
          ...
          // Broadcast to all users
          await broadcastService.broadcastNationalAlert(nationalAlert);
          console.log('✅ TEST ALERT: Outbreak broadcast completed successfully at 5:45 PM');
        } else {
          console.log('ℹ️ TEST ALERT: No national outbreak alert to broadcast at 5:45 PM');
        }
        
      } catch (error) {
        console.error('❌ Error in 5:45 PM test outbreak broadcast:', error);
        
        // Send error notification to admin (if configured)
        await this.notifyAdminOfError('5:45 PM Test Outbreak Broadcast Failed', error);
      }
    }, {
      timezone: "Asia/Kolkata"
    });

    console.log('🧪 TEST ALERT: Outbreak broadcast scheduled for 5:45 PM IST TODAY');
  }
  ...
  getStatus() {
    ...
    return {
      ...
      jobs: {
        ...
        testAlert: {
          schedule: '45 17 * * *',
          description: 'TEST ALERT: Outbreak broadcast at 5:45 PM IST TODAY',
